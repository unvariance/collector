...
## Summary
Add end-to-end tests that validate resctrl plugin behavior for Add/Remove events, pod group state and per-pod reconciliation counts, startup cleanup, and retries. Validate that a Pod initially synchronized with a subset of its containers (e.g., due to concurrent creation) is fully reconciled when late containers appear (in e2e test, cover this using `kubectl debug`, which will also validate _ephemeral containers_, containers added to a running Pod via `kubectl debug`).

Align tests with the implemented event model and APIs introduced while working on Issue #252 and its sub-issues:
...
- A test with the following flow:
  ...
  2) Post-start add container: After plugin start, add a new container in the pod using `kubectl debug` to add an ephemeral container; verify reconciliation adds tasks and counts improve.
...
- A test for capacity and retries:
  1) RMID exhaustion and caller-driven retry: Pre-fill resctrl capacity using a distinct prefix (so startup cleanup doesn’t remove them); verify group creation fails (`group_state = Failed`). Then free one resource and call `retry_group_creation` or `retry_all_once`; expect transition to `Exists(path)` and tasks assigned.

Wire tests into CI (`test-resctrl.yaml`):
...
## Shared Test Harness (already exists)
...
## Success Criteria
- Mocked tests run in CI using mocked resctrl FS and a test PID source.
- Hardware E2E passes on capable instances.
...
- Hardware path:
  ...
  - Post-start add Pod: create a new Pod after plugin start and verify group creation and task assignment (expect AddOrCreate with reconciliation counts and PID/task assignment).
  - Exhaustion test: pre-create many monitoring groups with a separate prefix until ENOSPC; verify `group_state = Failed`. Then delete one helper group and call `retry_group_creation` or `retry_all_once`; verify transition to `Exists(path)` and task assignment. Note: it might take a few second for the kernel to free the RMID after group deletion, let's call retry every second for up to 30s (fail if unsuccessful after that time). Note that even after cleaning up the pre-created groups, the system might need a while before more tests can run successfully. Run this test last in a separate step and document this in the workflow.
  - In exhaustion test, keep prefixes distinct so startup cleanup doesn’t remove the exhaustion helpers.
  - Use different prefixes for different tests to avoid interference. Ask for cleanup on start.
...
- Test inventory and deltas
  - In-crate unit/integration (mocked) in `crates/nri-resctrl-plugin/src/lib.rs`:
    ...
    - `test_reconcile_emits_counts` — already exists; initial synchronize counts. Expand to cover late-container reconciliation, where a container for a pre-existing pod is received AFTER synchronize(). Modify. Also rename to `test_synchronize_emits_counts`.
    - `test_run_pod_sandbox_creates_group_and_emits_event` — already exists; validates `RUN_POD_SANDBOX` and removal. Add container creation and removal. Modify.
  ...
  - E2E tests:
    - `crates/nri-resctrl-plugin/tests/integration_test.rs::test_resctrl_plugin_registers_with_nri` — exists; registration path. Keep. Need to enable this in CI under the KIND setup. Rename to "test_plugin_full_flow" and expand to cover:
      ...
      - Pod removal: remove preexisting Pod and post-registration Pod; verify `Removed` events and cleanup.
