...
fn regex_for_semver() -> regex::Regex {
    ...
    ONCE.get_or_init(|| regex::Regex::new(r"\b(\d+)\.(\d+)\.(\d+)\b").unwrap()).clone()
}

pub fn detect(opts: &Options) -> Result<Detection> {
    if let Mode::K3s = opts.mode { return Ok(Detection { env: EnvKind::K3s { version: detect_k3s_version(opts)? }, containerd_version: detect_containerd_version(opts)? }); }
    if let Mode::Containerd = opts.mode { return Ok(Detection { env: EnvKind::Containerd, containerd_version: detect_containerd_version(opts)? }); }

    // Auto mode
    if std::path::Path::new("/var/lib/rancher/k3s").exists() {
        info!("K3s installation detected");
        return Ok(Detection { env: EnvKind::K3s { version: detect_k3s_version(opts)? }, containerd_version: detect_containerd_version(opts)? });
    }
    info!("Plain containerd environment assumed");
    Ok(Detection { env: EnvKind::Containerd, containerd_version: detect_containerd_version(opts)? })
}
...
pub fn detect_containerd_version(opts: &Options) -> Result<Option<Version>> {
    let runner = default_runner(&opts.nsenter);
    // containerd --version
    if let Ok(out) = runner.run_ok("containerd", &["--version"]) { return Ok(parse_first_semver(&out)); }
    // containerd -v
    if let Ok(out) = runner.run_ok("containerd", &["-v"]) { return Ok(parse_first_semver(&out)); }
    ...
    if let Ok(out) = runner.run_ok("ctr", &["version"]) {
        ...
        for line in out.lines() {
            if let Some(stripped) = line.trim().strip_prefix("Version:") {
                if let Ok(v) = Version::parse(stripped.trim()) { return Ok(Some(v)); }
            }
        }
