...
pub enum RestartResult { NotRequested, NotSupported, Issued, Verified }
...
fn service_monotonic_start(runner: &crate::cmd::Runner, svc: &str) -> Option<u128> {
    ...
    if let Ok(out) = runner.run_ok("systemctl", &["show", svc, "-p", "ExecMainStartTimestampMonotonic"]) {
        for line in out.lines() {
            if let Some(val) = line.strip_prefix("ExecMainStartTimestampMonotonic=") {
                if let Ok(v) = val.trim().parse::<u128>() { return Some(v); }
            }
        }
    }
    None
...
pub fn restart_and_verify(service_hint: &str, opts: &Options) -> Result<RestartResult> {
    if !opts.restart { return Ok(RestartResult::NotRequested); }
    let runner = default_runner(&opts.nsenter);

    // Determine candidate services for hint
    let candidates: Vec<&str> = match service_hint { "k3s" => vec!["k3s", "k3s-agent"], _ => vec!["containerd"] };

    // Observe pre-restart timestamp for first available candidate
    let mut chosen: Option<&str> = None;
    let mut before: Option<u128> = None;
    for svc in &candidates {
        if let Some(ts) = service_monotonic_start(&runner, svc) { chosen = Some(svc); before = Some(ts); break; }
    }
    ...
    if !issued { warn!("Unable to restart service automatically"); return Ok(RestartResult::NotSupported); }
    ...
    if let Some(svc) = chosen {
        ...
        return Ok(if verified { RestartResult::Verified } else { RestartResult::Issued });
    }
