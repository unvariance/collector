...
pub async fn task_completion_handler<F, T, E>(
    future: F,
...
mod tests {
    ...
    async fn test_error_completion() {
        ...
        testing_logger::validate(|captured_logs| {
            ...
            assert_eq!(captured_logs[0].body, "error_task failed with error: TestError(\"test error\")");
        });
    }
    ...
    async fn test_panic_completion() {
        ...
        testing_logger::validate(|captured_logs| {
            assert_eq!(captured_logs.len(), 1);
            assert_eq!(captured_logs[0].level, log::Level::Error);
            assert!(captured_logs[0].body.starts_with("panic_task panicked or was cancelled:"));
    ...
    async fn test_multiple_handlers_independent_cancellation() {
        ...
        testing_logger::validate(|captured_logs| {
            ...
            let success_log = captured_logs.iter().find(|log| 
                log.body.contains("multi_task1 completed successfully")
            ).expect("Should have success log");
            assert_eq!(success_log.level, log::Level::Debug);
            
            // Find the error log
            let error_log = captured_logs.iter().find(|log| 
                log.body.contains("multi_task2 failed with error")
    ...
    async fn test_task_name_variations() {
        ...
        let test_cases = vec![
            ...
            ("complex-task-name", "complex-task-name completed successfully"),
            ("TaskWithCamelCase", "TaskWithCamelCase completed successfully"),
    ...
    async fn test_complex_error_types() {
        ...
        testing_logger::validate(|captured_logs| {
            assert_eq!(captured_logs.len(), 1);
            assert_eq!(captured_logs[0].level, log::Level::Error);
            assert!(captured_logs[0].body.starts_with("complex_task failed with error:"));
